...
class Controller():
    def __init__(self, name, grip):
        ...
        self.__joints_com = []
        self.__joints_com.append(rospy.Publisher('/' + name + '/trajectory_controller/command', JointTrajectory, queue_size=1))
...
# --------------------- Move the desired homogeneus transform -----------------
    def __move(self, T):
        ...
        msg.points.positions = self.__qp
        msg.points.velocity = [0,0,0,0,0,0]
        msg.points.effort = [0,0,0,0,0,0]
        msg.points.duration = 0.01
...
# ---------------- Home position ----------------
    def home(self, key):
        if key == keyboard.Key.esc:
            ...
            msg.joint_names = ["shoulder_pan_joint", "shoulder_lift_joint", "elbow_joint", "wrist_1_joint", "wrist_2_joint", "wrist_3_joint"]
            print("BBBBBB")
            msg.points.positions = [0.0, -1.57, 1.57, -1.57, -1.57, 0.0]
            print("CCCCCCC")
            msg.points.velocities = [0,0,0,0,0,0]
            print("AAAAAAAAAA")
            msg.points.accelerations = [0,0,0,0,0,0]
            print("DDDDDDD")
            msg.points.effort = [0,0,0,0,0,0]
            print("EEEEEE")
            msg.points.duration = 1
            print("TTTTT")
            self.__joints_com[0].publish(msg)
            print("XXXXXXX")
